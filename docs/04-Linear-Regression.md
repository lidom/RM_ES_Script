...
**Theorem** *The OLS estimator* $\mathbf{b}$
...
-   *(Gauss-Markov Theorem) is efficient in the class of all linear unbiased estimators. That is, for any unbiased estimator*
...
**Theorem** *Unbiasedness of $s^2$*

*Under Assumptions 1.1-1.4, we have that:* $$\mathbb{E}(s^2|\mathbf{X})=\sigma^2,$$ *and hence* $\mathbb{E}(s^2)=\sigma^2$, *provided that* $n>K$ *(otherwise $s^2$ isn't well defined).*
...
\end{array}
$$ 
it follows by [Kolmogorov's strong law of large numbers](https://www.statlect.com/asymptotic-theory/law-of-large-numbers)
that 
\begin{align*}
...
2nd Part: \
By the [Continuous Mapping Theorem](https://www.statlect.com/asymptotic-theory/continuous-mapping-theorem)
we have that also 
\begin{align*}
